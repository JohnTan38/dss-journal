...
export default function KnowledgeSection({ subSection, theme, className }: KnowledgeSectionProps) {
  const headerStyle = getSectionHeaderStyle(theme, 'knowledge');
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className={cn('flex items-start gap-3 rounded-r-xl border-l-4 p-4', headerStyle.backgroundClass, headerStyle.borderClass)}>
        <Brain className={cn('mt-0.5 h-5 w-5 flex-shrink-0', headerStyle.iconClass)} />
        <div className="space-y-1">
          <SectionBadge label={subSection.label} variant="knowledge" className={theme.knowledgeBadgeClass} />
